...
- **A detection parses and evaluates itself.** `IDetection::inspect(data, size, meta)` returns a `Verdict` or `nullopt`; `DetectLoop` walks the submitted list in order and the first verdict wins. Nothing outside a detection knows what an event is, and there is no downcast anywhere on the path.
- **List order is priority order, and it lives in the hook.** A ClientHello can satisfy several detections at once and only the first verdict is emitted, so `xdp_tls` puts its two enforcing detections ahead of its two alert-only ones. The loop logs which index claimed each record, so the ordering is observable at runtime.
