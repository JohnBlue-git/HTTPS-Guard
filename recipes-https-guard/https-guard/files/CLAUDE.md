...
One `CMakeLists.txt` per concern (`actions_lib`, `detections_lib`, `programs_lib`), plus this directory's root `CMakeLists.txt` tying them into the final executables (`https_guardd`, `action_runner`) and the BPF object. Adding a source file to one concern's directory should never require touching another concern's `CMakeLists.txt`.
