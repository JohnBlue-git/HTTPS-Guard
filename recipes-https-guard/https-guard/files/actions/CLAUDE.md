...
`HttpGuardProgram::ringBufferHandler` (in `../programs/core/`) only reaches into here when a detector's `Verdict::actionable` is true *and* the event carries a non-zero `remote_ip_v4` (the peer's address — `hg_event`'s fields are named by role, `local_*`/`remote_*`, not by direction; see its header for why) — uprobe events need `ProcPeerResolver` to have resolved a socket first (not guaranteed; logged as a warning if it fails), XDP events always carry one from the packet headers. On an actionable event: `BlockTcpAction` then `BlocklistAddAction`, both dispatched, then `LogAction` (always, actionable or not).
