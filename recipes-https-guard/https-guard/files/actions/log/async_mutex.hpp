...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<asio::posix::stream_descriptor>> stream_cache_;
...
public:
    ...
    public:
        ...
        LockedStream(AsyncFileStreamManager* owner, std::string filename,
                     std::shared_ptr<asio::posix::stream_descriptor> stream)
            : owner_(owner)
        ...
        asio::posix::stream_descriptor& stream() const
        {
            return *stream_;
        }
    ...
    private:
        ...
        std::shared_ptr<asio::posix::stream_descriptor> stream_;
    };
    ...
    boost::asio::awaitable<LockedStream> acquire_stream(const std::string& filename)
    {
        ...
        std::shared_ptr<asio::posix::stream_descriptor> stream_ptr;
        {
            std::lock_guard lock(mtx_);
        ...
        if (!stream_ptr) {
            ...
            const int fd = ::open(filename.c_str(), O_WRONLY | O_CREAT | O_APPEND | O_NONBLOCK, 0644);
            if (fd < 0) {
                unlock(filename);
                co_return LockedStream{};
            }

            stream_ptr = std::make_shared<asio::posix::stream_descriptor>(ex, fd);
            std::lock_guard lock(mtx_);
