 ...
 * and invoked through `EventMeta::ensurePeerResolved()`, which memoises so
...
public:
    ...
    /**
     * Fills meta's local_ and remote_ fields. Returns whether a connection
     * was unambiguously identified; on false the fields are left untouched
     * (zeroed), which callers treat as "do not enforce".
     */
    virtual bool resolvePeer(EventMeta& meta) const noexcept = 0;
};
