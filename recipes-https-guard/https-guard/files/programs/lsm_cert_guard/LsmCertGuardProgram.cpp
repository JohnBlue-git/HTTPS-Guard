...
std::optional<hg_event> LsmCertGuardProgram::parseEvent(const void* data, size_t size) const noexcept
{
    ...
    evt.cert_shadow_mode = (raw->shadow_mode != 0);
    ...
    evt.cert_identity_mismatch = (evt.real_exe_path != kExpectedBmcwebExe);

    std::cout << "https_guard: certificate access observed: process='" << evt.process
              << "' (PID " << evt.pid << "), real_exe='" << evt.real_exe_path
              << "', identity_mismatch=" << evt.cert_identity_mismatch << "\n";
