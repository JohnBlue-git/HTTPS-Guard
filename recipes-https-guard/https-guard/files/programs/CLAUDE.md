...
The shared [`PROGRAM.md`](../../../../docs/PROGRAM.md) covers `BpfProgram`, `HttpGuardProgram`, the
one-object invariant and each hook's attach specifics. Detection rationale moved
to the specific markdown document in `../detections/<family>/`,
...
7. Any new detection is a directory under `../detections/<family>/` holding its event struct, its rule, its `IDetection` implementation and its specific markdown document — then one entry in this hook's list. Nothing about it goes in this directory, and `main.cpp` does not change.
